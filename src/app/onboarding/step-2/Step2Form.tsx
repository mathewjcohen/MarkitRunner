...
import { clearChannelsForBusiness, createChannel } from '@/actions/channels'
...
interface Props {
  businessId: string
  initialSelected: ChannelType[]
}

export function Step2Form({ businessId, initialSelected }: Props) {
  const router = useRouter()
  const [selected, setSelected] = useState<ChannelType[]>(initialSelected)
  ...
  async function handleNext() {
    ...
    setLoading(true)
    setError(null)
    const clearResult = await clearChannelsForBusiness(businessId)
